...
struct RewardParams {
    // --- Racing / stabilization parameters ---
    ...
    // Shared observation normalization bounds
    ...
    // Perception / camera parameters
    double PERCEPTION_ALPHA2;
    double PERCEPTION_ALPHA3;
    double PERCEPTION_ALPHA1;
    double CAMERA_TILT_ANGLE_DEG;

    // Corner / gate observation bounds
    ...
    // --- Interception task parameters ---
    double INTERCEPTION_CAPTURE_RADIUS;
    double INTERCEPTION_MAX_EPISODE_TIME_S;
    double INTERCEPTION_DISTANCE_SCALE;
    double INTERCEPTION_DIST_DELTA_SCALE;
    double INTERCEPTION_VEL_CLOSING_SCALE;
    double INTERCEPTION_TIME_PENALTY_PER_STEP;
    double INTERCEPTION_SUCCESS_REWARD;
    double INTERCEPTION_CRASH_PENALTY;
    ...
    RewardParams(const std::string& filepath) {
        ...
        PERCEPTION_ALPHA1             = config["PERCEPTION_ALPHA1"].as<double>();
        ...
        RESET_VEL_GATE_START_PERTURB_BOUND =
            config["RESET_VEL_GATE_START_PERTURB_BOUND"].as<double>();
        RESET_ATT_RP_GATE_START_PERTURB_BOUND =
            config["RESET_ATT_RP_GATE_START_PERTURB_BOUND"].as<double>();
        RESET_ATT_YAW_GATE_START_PERTURB_BOUND =
            config["RESET_ATT_YAW_GATE_START_PERTURB_BOUND"].as<double>();
        RESET_ATT_YAW_GLOBAL_START_PERTURB_BOUND =
            config["RESET_ATT_YAW_GLOBAL_START_PERTURB_BOUND"].as<double>();
        RESET_OME_GATE_START_PERTURB_BOUND =
            config["RESET_OME_GATE_START_PERTURB_BOUND"].as<double>();
        DISCOUNT_FACTOR_GAMMA         = config["DISCOUNT_FACTOR_GAMMA"].as<double>();
        INITIAL_RESET_PITCH_TILT_DEGREES =
            config["INITIAL_RESET_PITCH_TILT_DEGREES"].as<double>();
        ...
        // Helper for optional interception keys (supports BOTH flat keys and nested 'interception:' group)
        auto nodeAtPath = [&](const YAML::Node& root, const std::string& path) -> YAML::Node {
            // direct (flat) lookup first
            if (root[path]) return root[path];

            // dotted traversal: e.g. "interception.capture_radius"
            YAML::Node n = root;
            size_t start = 0;
        ...
        auto getDoubleAny = [&](std::initializer_list<const char*> keys, double def, const char* printable) -> double {
            for (const char* k : keys) {
                YAML::Node n = nodeAtPath(config, std::string(k));
                if (!n) continue;
                try {
                    return n.as<double>();
                } catch (const std::exception& e) {
                    std::cout << "[RewardParams][WARN] Key '" << k
                              << "' exists but is not numeric (" << e.what()
                              << "). Using default=" << def << " for " << printable << std::endl;
                    return def;
                }
            }
            return def;
        ...
        INTERCEPTION_CAPTURE_RADIUS = getDoubleAny(
            {"INTERCEPTION_CAPTURE_RADIUS", "interception.capture_radius"}, 3.0, "INTERCEPTION_CAPTURE_RADIUS");
        INTERCEPTION_MAX_EPISODE_TIME_S = getDoubleAny(
            {"INTERCEPTION_MAX_EPISODE_TIME_S", "interception.max_episode_time_s"}, 30.0, "INTERCEPTION_MAX_EPISODE_TIME_S");
        INTERCEPTION_DISTANCE_SCALE = getDoubleAny(
            {"INTERCEPTION_DISTANCE_SCALE", "interception.distance_scale"}, 0.6, "INTERCEPTION_DISTANCE_SCALE");
        INTERCEPTION_DIST_DELTA_SCALE = getDoubleAny(
            {"INTERCEPTION_DIST_DELTA_SCALE", "interception.dist_delta_scale"}, 2.5, "INTERCEPTION_DIST_DELTA_SCALE");
        INTERCEPTION_VEL_CLOSING_SCALE = getDoubleAny(
            {"INTERCEPTION_VEL_CLOSING_SCALE", "interception.vel_closing_scale"}, 1.0, "INTERCEPTION_VEL_CLOSING_SCALE");
        INTERCEPTION_TIME_PENALTY_PER_STEP = getDoubleAny(
            {"INTERCEPTION_TIME_PENALTY_PER_STEP", "interception.time_penalty_per_step"}, 0.0, "INTERCEPTION_TIME_PENALTY_PER_STEP");
        INTERCEPTION_SUCCESS_REWARD = getDoubleAny(
            {"INTERCEPTION_SUCCESS_REWARD", "interception.success_reward"}, 30.0, "INTERCEPTION_SUCCESS_REWARD");
        INTERCEPTION_CRASH_PENALTY = getDoubleAny(
            {"INTERCEPTION_CRASH_PENALTY", "interception.crash_penalty"}, -30.0, "INTERCEPTION_CRASH_PENALTY");

// Load vector types using the helper loadVector<>() defined above
    ...
    // Default ctor if someone wants to fill it manually
