...
policy_dir     = os.path.join(os.getcwd(), 'policies_intercept')   # FIX: match eval scripts
...
BEST_POLICY_PATH = os.path.join(policy_dir, 'best_intercept')       # FIX: match eval scripts
...
def train_eval():
    ...
    # Optional: deterministic evalMode for the TF-Agents eval metric
    if hasattr(base_py_eval_env, "setEval"):
        base_py_eval_env.setEval()

    # Greedy eval env: either deterministic or randomized (explicit)
    ...
    agent.initialize()
    # Ensure TensorBoard summary step is int64 to avoid WriteSummary dtype errors
    print(f"[train] train_step_counter dtype: {agent.train_step_counter.dtype}")
    try:
    ...
    for i in range(num_iterations):
        ...
        if current_step > 0 and current_step % eval_interval == 0:
            ...
            # TF-Agents metric eval (deterministic env if setEval above)
            ...
            # Greedy eval (explicit deterministic vs randomized)
            ...
            # Save best policy by TF-Agents avg_return
