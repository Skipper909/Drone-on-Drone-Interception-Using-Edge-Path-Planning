                                               ...
                                               int drone_idx) const
{
    ...
    // -------------------------------------------------------------------------
    // 3) Extra near-target shaping with two zones: 25 m, 10 m, 5 m and anti-loitering
    // -------------------------------------------------------------------------
    ...
    if (dist < NEAR_RADIUS) {
        ...
        double near_bonus  = 0.10 * success_reward_;  // keep your current 25%
    ...
    if (dist < INNER_RADIUS) {
        ...
        double inner_bonus = 0.10 * success_reward_;   // 10% of success reward
    ...
    // -------------------------------------------------------------------------
    // Anti-loitering: inside 10 m, not getting closer -> small penalty
    // -------------------------------------------------------------------------
    constexpr double ANTI_LOITER_RADIUS = 10.0;
    constexpr double ANTI_LOITER_EPS = 0.01;   // ~1 cm improvement threshold per step

    if (dist < ANTI_LOITER_RADIUS) {
        if (delta < ANTI_LOITER_EPS) {
            // Small per-step penalty for hovering without making progress
            double anti_loiter_penalty = 0.01 * success_reward_;  // ~1% of success reward
