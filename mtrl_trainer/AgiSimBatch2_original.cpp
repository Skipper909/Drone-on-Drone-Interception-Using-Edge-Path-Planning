...
namespace agi_env {
    ...
    reward_params_(sim_base_dir + "/reward_params.yaml") {
        ...
        available_tasks_.push_back(std::make_shared<TaskInterceptionMoving>());
        ...
        vio_drift_offsets_.assign(num_drones_, Vector<6>::Zero());
        ...
        for (int i = 0; i < num_drones_; ++i) {
            try {
                ...
                std::cerr << "Error initializing simulator instance " << i << ": " << e.what() << std::endl;
                throw;
            }
        }

        // Initialize random number generators for thread safety. Note: we currently have one generator per thread and not per drone which might be subobtimal
        int max_threads = 1;
#ifdef _OPENMP
...
#pragma omp parallel for schedule(static)
        for (int i = 0; i < num_drones_; ++i) {
            ...
            bool final_done = done_from_sim_status || task_result.missed_goal || evalMode_ && task_result.lap_completed;
