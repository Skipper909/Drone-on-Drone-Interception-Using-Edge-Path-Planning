...
struct RewardParams {
    ...
    RewardParams(const std::string& filepath) {
        ...
        // Helper for optional interception keys
        auto getOrDefault = [&](const std::string& key, double def) {
            if (config[key]) {
        ...
        INTERCEPTION_CAPTURE_RADIUS =
            getOrDefault("interception.capture_radius", 3.0);
        INTERCEPTION_MAX_EPISODE_TIME_S =
            getOrDefault("interception.max_episode_time_s", 30.0);
        INTERCEPTION_DISTANCE_SCALE =
            getOrDefault("interception.distance_scale", 0.6);
        INTERCEPTION_DIST_DELTA_SCALE =
            getOrDefault("interception.dist_delta_scale", 2.5);
        INTERCEPTION_VEL_CLOSING_SCALE =
            getOrDefault("interception.vel_closing_scale", 1.0);
        INTERCEPTION_TIME_PENALTY_PER_STEP =
            getOrDefault("interception.time_penalty_per_step", 0.0);
        INTERCEPTION_SUCCESS_REWARD =
            getOrDefault("interception.success_reward", 30.0);
        INTERCEPTION_CRASH_PENALTY =
            getOrDefault("interception.crash_penalty", -30.0);
