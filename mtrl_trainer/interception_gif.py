...
from matplotlib.animation import FuncAnimation, PillowWriter  # add near other imports at top


def make_interception_gif(drone_traj, target_traj,
                          output_path="interception.gif",
                          fps=20,
                          frame_stride=2):
    """
    Create a 3D GIF of a single interception trajectory.

    drone_traj and target_traj are lists of [x, y, z] points in world coordinates.
    """
    if not drone_traj or not target_traj:
        print("[gif] Empty trajectory, skipping GIF.")
        return

    import numpy as np
    import matplotlib.pyplot as plt

    drone = np.asarray(drone_traj, dtype=np.float32)
    target = np.asarray(target_traj, dtype=np.float32)
    ...
    if min_len < 2:
        print("[gif] Not enough points to animate (len < 2), skipping GIF.")
        return

    # Optional subsampling to keep GIF size reasonable
    ...
    all_pts = np.vstack([drone, target])
    margin = 10.0
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.set_title("Interception Animation")
    ax.set_xlabel("X-axis (m)")
    ax.set_ylabel("Y-axis (m)")
    ax.set_zlabel("Z-axis (m)")
    ...
    (drone_line,) = ax.plot([], [], [], "b-", linewidth=1.5, label="Drone")
    (target_line,) = ax.plot([], [], [], "r--", linewidth=1.0, label="Target")
    (drone_pt,) = ax.plot([], [], [], "bo", markersize=5)
    (target_pt,) = ax.plot([], [], [], "rs", markersize=5)
    ...
    def init():
        ...
        drone_pt.set_data([], [])
        drone_pt.set_3d_properties([])
        target_pt.set_data([], [])
        target_pt.set_3d_properties([])
        return drone_line, target_line, drone_pt, target_pt

    def update(frame_idx):
        xs_d, ys_d, zs_d = drone[: frame_idx + 1].T
        xs_t, ys_t, zs_t = target[: frame_idx + 1].T

        drone_line.set_data(xs_d, ys_d)
        drone_line.set_3d_properties(zs_d)

        target_line.set_data(xs_t, ys_t)
        target_line.set_3d_properties(zs_t)

        drone_pt.set_data([xs_d[-1]], [ys_d[-1]])
        drone_pt.set_3d_properties([zs_d[-1]])

        target_pt.set_data([xs_t[-1]], [ys_t[-1]])
        target_pt.set_3d_properties([zs_t[-1]])
    ...
    ani = FuncAnimation(
        ...
        init_func=init,
        blit=False,              # blit + 3D = pain
        interval=1000.0 / fps,
    )

    print(f"[gif] Saving GIF to '{output_path}' (fps={fps})...")
...
def main():
    ...
    visualize_track_layout(
        ...
        # --- Create GIF from the same trajectories we just visualized ---
    if (policy_run_attempted and
        all_drone_trajectories is not None and
        all_target_trajectories is not None and
        len(all_drone_trajectories) > 0 and
        len(all_target_trajectories) > 0 and
        len(all_drone_trajectories[0]) > 1 and
        len(all_target_trajectories[0]) > 1):

        gif_drone_traj = all_drone_trajectories[0]
        gif_target_traj = all_target_trajectories[0]
        make_interception_gif(gif_drone_traj, gif_target_traj,
                              output_path="interception.gif",
                              fps=20,
                              frame_stride=2)
    else:
        print("[gif] Skipping GIF creation (no valid trajectories).")
