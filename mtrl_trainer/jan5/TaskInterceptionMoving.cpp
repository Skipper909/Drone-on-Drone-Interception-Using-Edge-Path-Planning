                                               ...
                                               int drone_idx) const
{
    ...
    if (effective_progress > 0.0) {
        reward += dist_delta_scale_ * effective_progress;
    }
    ...
    if (delta_dist > 0.0) {
        reward += dist_delta_scale_ * delta_dist;
    }
    if (delta_dca > 0.0) {
        reward += (0.5 * dist_delta_scale_) * delta_dca;
    }
    ...
    // Near-target shaping (keeps training from collapsing)
    constexpr double FAR_RADIUS   = 25.0;
    constexpr double NEAR_RADIUS  = 10.0;
    constexpr double INNER_RADIUS = 3.0;

    if (dist < FAR_RADIUS) {
        const double factor = (FAR_RADIUS - dist) / FAR_RADIUS;
        reward += (0.05 * success_reward_) * factor;
    }
    if (dist < NEAR_RADIUS) {
        const double factor = (NEAR_RADIUS - dist) / NEAR_RADIUS;
        reward += (0.15 * success_reward_) * factor; // your requested 10m bonus style
    }
    if (dist < INNER_RADIUS) {
        const double factor = (INNER_RADIUS - dist) / INNER_RADIUS;
        reward += (0.10 * success_reward_) * factor;
    }

    // Anti-loiter inside 10m: require actual progress
    constexpr double ANTI_R = 10.0;
    constexpr double ANTI_EPS = 0.01;
    if (dist < ANTI_R) {
        if (delta_dist < ANTI_EPS && delta_dca < ANTI_EPS) {
            reward -= 0.01 * success_reward_;
        }
    }
